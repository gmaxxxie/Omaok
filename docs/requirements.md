...
- **ADR-012（2026-09-03）**：批量扩展 18 个动作，全部走 Omarchy 常规命令：①无参 toggle（`set_nightlight`→`omarchy-shell nightlight enable|disable|toggle`、`set_bluetooth`→`omarchy bluetooth power`、`set_wifi`→`nmcli radio wifi`、`set_touchpad`→`omarchy toggle touchpad`、`toggle_dnd`→`omarchy toggle notification silencing`、`toggle_mic`→`omarchy audio input mute`、`toggle_bar`、`open_clipboard`/`open_emoji`→`omarchy menu`）；②参数型（`set_reminder`→`omarchy reminder <min> <msg>`，规则/AI 提取分钟与消息，支持 zh/en/半小时/小时换算；`brightness_up/down`→`omarchy brightness display +10%/10%-`；`set_power_mode`→`omarchy powerprofiles set`）；③破坏性（`shutdown/reboot/logout`→`omarchy system *`、`screen_record_start/stop`→`omarchy capture screenrecording`，全部 confirm_required）。84 单测全绿。关键坑：nightlight IPC 方法名是 enable/disable/toggle 而非 on/off。
- **ADR-013（2026-09-03）**：**速度优化**（实测）：①STT 模型 fp32→**small-int8**（894MB→229MB，加载+推理 3.0s→**2.1s**）——助手用专用 voxtype 配置 `$XDG_RUNTIME_DIR/omarchy-voice-control/voxtype-assistant.toml`（`voxtype -c` 隔离，**不动**用户听写配置 ~/.config/voxtype/config.toml；stt.model 可改回 small-fp32）；②**常驻 ai-daemon**（`omarchy-voice-control ai-daemon`）：持有一个热 pi RPC 进程，Unix socket JSON 服务，AI 路径 5s→**2.5s**；客户端首用自动拉起（`shutil.which("omarchy-voice-control")` 找入口，勿用 sys.argv[0]——测试 harness 下无效）；空闲 ai.idle_secs(900s) 自动退出、下次自动重启；失败回退一次性 `_rpc_prompt`。③AI prompt 精简（去掉 40 条 omarchy 命令段）。86 单测全绿。
- **ADR-014（2026-09-03）**：**操作记忆缓存**（`~/.config/omarchy/voice-control/memory.json`）：记住成功解析的「语音→动作 draft」，规则未命中、走 AI **之前**先查记忆（精确匹配 + 保守模糊 ≥0.92），重复/近似指令**跳过 AI**（实测 7.8s→**0.1s**）；缓存 draft 每次仍经 resolver（blocklist 变更即时生效）+ policy（白名单/风险/确认）全关卡；上限 200 条、按规范化文本去重。92 单测全绿。**坑**：改 config.json 时全局 `.replace("model": null,)` 误把 `ai.model` 也改成 small-int8，导致 `pi --model small-int8` 秒退挂起——AI model 必须留 null。
- **ADR-015（2026-09-03）**：**产品命名 omaok**（用户指定）；GitHub 仓库 `gmaxxxie/Omaok`。manifest displayName / 弹窗标题 / README 改为 **omaok**；插件 id 保持 `max.voice-control`（避免重装破坏）。
